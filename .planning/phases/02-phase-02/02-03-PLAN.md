...
@.planning/phases/02-phase-02/02-02-SUMMARY.md
@gdoc/cli.py
@gdoc/api/drive.py
@gdoc/format.py
</context>
...
<task type="auto">
  ...
  <action>
    ...
    **5. Update `tests/test_cli.py`:**
    ...
    - **Update `TestErrorFormat.test_stub_error_prefix`:** Currently runs `run_gdoc("ls")` and asserts stderr starts with `"ERR: "`. Since `ls` is no longer a stub, change to `run_gdoc("edit", "doc", "old", "new")` (a still-stubbed command).
    - **Update `TestMutuallyExclusiveFlags`:** Several tests run `ls` and assert returncode == 4 (expecting stub behavior). Since `ls` is now a real command that would hit the Google API in subprocess tests, switch these to still-stubbed commands:
      - `test_json_accepted`: Change from `run_gdoc("--json", "ls")` to `run_gdoc("--json", "edit", "doc", "old", "new")`, assert returncode == 4
      - `test_verbose_accepted`: Change from `run_gdoc("--verbose", "ls")` to `run_gdoc("--verbose", "edit", "doc", "old", "new")`, assert returncode == 4
      - `test_verbose_after_subcommand`: Change from `run_gdoc("ls", "--verbose")` to `run_gdoc("edit", "doc", "old", "new", "--verbose")`, assert returncode == 4
      - The conflict tests (`test_json_and_verbose_conflict`, `test_json_and_verbose_conflict_after_subcommand`, `test_json_before_verbose_after_subcommand_conflict`, `test_verbose_before_json_after_subcommand_conflict`) assert returncode == 3 or != 0, which are parser errors that happen before the handler runs. These can keep using `ls` since argparse will reject the flags before any API call. However, for consistency, switch these to `edit` as well.
  </action>
  <verify>
...
<output>
After completion, create `.planning/phases/02-phase-02/02-03-SUMMARY.md`
</output>

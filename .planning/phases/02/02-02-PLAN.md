...
must_haves:
  truths:
    ...
    - "gdoc info DOC_ID prints Title, Owner, Modified, Words in terse mode"
...
<task type="auto">
  ...
  <action>
    ...
    **3. Add `cmd_info` handler:**
    ...
    - Get word count: `text = export_doc(doc_id, mime_type="text/plain")` then `word_count = len(text.split())`
    - Extract fields:
...
<task type="auto">
  ...
  <done>
    All cat tests pass (7+ tests covering markdown/plain/json/comments-stub/errors). All info tests pass (10+ tests covering terse/verbose/json/owner-fallback/errors). Full test suite green.
  </done>
</task>
...
5. `tests/test_info.py` has 10+ tests passing
...
- cmd_info outputs Title/Owner/Modified/Words in terse mode
- cmd_info --verbose adds Created/Last editor/Type/Size
- cmd_info --json outputs flat metadata JSON

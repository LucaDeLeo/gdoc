...
1. **Modify `list_comments` signature** to add `include_resolved: bool = True` and `include_anchor: bool = False` parameters.
   ...
   - Add `createdTime` to the base `fields` string (after `resolved,`): the `comments` command needs it for the date column (`YYYY-MM-DD`). The existing `modifiedTime` is kept for pre-flight delta logic.
...
- `list_comments` accepts `include_resolved` and `include_anchor` params; filters client-side; does NOT pass `includeResolved` to API. Fields include both `createdTime` (for display date) and `modifiedTime` (for pre-flight delta).
